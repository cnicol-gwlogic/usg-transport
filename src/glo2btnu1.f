...
!          OPEN(ISPCUN, FILE = CFILENAME, FORM = 'BINARY',  !kkz - use FORM from openspec.inc rather than hardwiring BINARY or UNFORMATTED
...
C9-----------COMPUTE MASS ON AIR-WATER INTERFACE
            IF(IAW_ADSORB.EQ.1)THEN      !LINEAR 
...
C11------------ADD UP ALL THE TERMS
     ...
     1        + MASLOPR + MASLOAWI 
        ...
        ENDIF
      ENDDO
C12--------FOR IMMOBILE COMPONENTS COMPUTE MASS AS ONLY BEING IN THE TOTAL VOLUME
...
C     ------------------------------------------------------------------
     ...
     1   ISOLUBILITY,SOLLIM,SOLSLOPE,IAW_ADSORB
...
C11B-------PREPARE CONSTANT PART OF ADSORPTION TERM
        ...
        ENDDO
C---------------------------------------------------------
C12-----READ FREUNDLICH EXPONENT OR LANGMUIR COEFFICIENT OF EACH SPECIES 
...
C11A-------PREPARE CONSTANT PART OF ADSORPTION TERM
        ...
        ENDDO
C---------------------------------------------------------
C12-----READ FREUNDLICH EXPONENT OR LANGMUIR COEFFICIENT FOR EACH SPECIES
        IF(IADSORB.EQ.2.OR.IADSORB.EQ.3)THEN
          DO K=1,NLAY
            NNDLAY = NODLAY(K)
            NSTRT = NODLAY(K-1)+1
...
C     ------------------------------------------------------------------
     ...
     1    IAW_ADSORB,ISATADSORB,ADSORB,IADSORB
...
C5A ----FOR MULTIFILE OUTPUT OPTION, ADD COMPONENT SPECIES TO ROOT NAME AND OPEN FILE ON ISPCFM AND GO TO END OF FILE
      IF(IMULTI.NE.0)THEN
        ...
        ISTATUS = 'OLD'
        FORMC = 'FORMATTED'
        IF(CSPCFM.EQ.' ') FORMC = 'UNFORMATTED'
...
CC        ENDIF
C
!        OPEN(ICLNCN, FILE = CFILENAME, FORM = 'BINARY',   !kkz - use FORM from openspec.inc rather than hardwiring BINARY or UNFORMATTED
